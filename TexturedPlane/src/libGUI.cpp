...
void SetTrackbarPosition(HWND hwnd, int position)
{
    SendMessageW(hwnd, TBM_SETPOS, TRUE, position);
}
...
void DisplayError(const char* errorMesssage)
{
    DisplayError(ToWChar(errorMesssage));
}
...
void SetStatusValue(HWND statusBar, const char* value, int part)
{
    SetStatusValue(statusBar, ToWChar(value), part);
}